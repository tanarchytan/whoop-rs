...
| `pipeline.rs` | the staging path as **19 numbered steps** with `run_to(input, cfg, p, upto)`, a per-step artifact digest and a per-prefix digest, plus `StepId::deps()` (the DAG) and `StepId::feedback()` (the one forward reach, `Anchor` probe-decoding). 5 steps carry an artifact today; 3..=9 fold into `Assemble`. `SleepConfig` is the swap seam — `emit` (`EmitCfg::{V2, V2PlusCardiac}`) and `decode` (`DecodeCfg::{Viterbi, Conditioned, PosteriorMarginal, MarkovLoss}`); the anchor probe is always the shipped decoder |
| `features.rs` | the per-epoch feature vector (`Features`, `Cardiac`, `EPOCH_S`) every emission reads |
| `cardiac.rs` | interval order statistics over one beat window — seven quantiles absolute, seven detrended, mean HR, RMSSD/pNN50/mean\|ΔRR\|. One producer for the stager and the screening harnesses, so what is measured is what runs. Owns `WINDOW_S` (270 s, nine epochs) and re-exports the beat reconstruction the windows are taken over. Reached from a staging path through `cardiac_emit.rs` |
| `cardiac_emit.rs` | the fourteen percentile columns of `cardiac.rs` AS AN EMISSION: v2's row plus `lambda` times a fitted per-class linear discriminant over them. `columns` builds them inside the pipeline from the night's own R-R on v2's epoch grid, so the rows a harness fits on and the rows the emission reads come from one function; an epoch whose window holds too few beats keeps v2's row exactly. The map is read out of an `lda::Lda` by probing its basis — no second classifier — and is fitted held out by recording, never here. `STAGE_ORDER` and the truth encoding are different orders and `delta` maps by name. Reached through `EmitCfg::V2PlusCardiac`; `lambda_milli: 0` is v2 row for row, the built-in null. Measured on the border card at lambda 0.5 / 1.0 / 2.0 beside its permuted twin: at lambda 1.0 balanced accuracy +0.021 DREAMT and +0.028 AAUWSS against a permuted -0.005, deep recall 0.172 -> 0.281 and 0.449 -> 0.575 while the permuted arm stays inert. No kappa4 gain on any cohort - AAUWSS's +0.004 goes negative on both disjoint halves - the calling share outruns every recall it lifts, and deep bouts move further from truth (7.4 -> 12.5 min against 2.4). **Not on the shipped path** |
...
| `markov_loss.rs` | a staging loss that prices misplaced BOUNDARIES as well as misclassified epochs — per-class epoch cost, invented-boundary cost, missed-boundary cost. Its per-class `fc` now decodes through `DecodeCfg::MarkovLoss`, charged on the TRUE class so a dearer class is called MORE; `ft` and `fh` price a PAIR of epochs and no per-epoch rule can charge them, so only `fc` has ever been scored. `inverse_prevalence` builds the weighting a class-balanced objective implies by COUNTING classes, at geometric mean 1 because only the ratios decide; `geometric_scale` walks the path from unit costs to it. Measured on the border card at powers 0, 0.5 and 1 beside a permuted-weight twin: balanced accuracy rises monotonically on all three cohorts and beats the permuted twin on five of five slices, kappa4 falls on all three, every gaining class is sub-proportional in its calling share, and the weighting is aimed at deep on DREAMT but at WAKE on both other cohorts. **Not on the shipped path** |
...
**1246 workspace tests, 0 clippy** (measured 2026-09-18; re-derive, never carry forward)**.**
